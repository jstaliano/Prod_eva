...
var inputTelefone = document.getElementById("placa");
...
var inputCelular = document.getElementById("hora");
...
if (linkExclusao != null) { 
	for ( var i = 0; i < linkExclusao.length; i++ ) {
		(function(i){
			var Id_Pedido = linkExclusao[i].getAttribute('rel');

			if (linkExclusao[i].addEventListener){
		    	linkExclusao[i].addEventListener("click", function(){confirmaExclusao(Id_Pedido);});
			}else if (linkExclusao[i].attachEvent) { 
				linkExclusao[i].attachEvent("onclick", function(){confirmaExclusao(Id_Pedido);});
			}
		})(i);
	}
}
...
function confirmaExclusao(id){
	retorno = confirm("Deseja Calcelar esse Pedido de Guincho?")
	...
	if (retorno){
	    ...
	    //var cancelamento = document.createElement("form");
	    formulario.action = "action_cliente.php";
	    formulario.method = "post";
	    //do {
			cancelamento = prompt ("Coloque o motivo do Cancelamento do Guincho!");
		//} while (cancelamento == null || cancelamento == "");
	    ...
	    var inputcancela = document.createElement("input");
	    inputcancela.type = "hidden";
	    inputcancela.value = cancelamento;
	    inputcancela.name = "cancelaguincho";
	    formulario.appendChild(inputcancela);
